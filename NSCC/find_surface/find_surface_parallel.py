...
PI = np.pi
angstrom = 1e-10
root_folder = Path(os.path.realpath(__file__)).parents[2]
dataset_name = "test"
data = pd.read_csv(os.path.join(root_folder, "data", "{}.csv".format(dataset_name)))
...
def vsa(atom_type, pos, cell_matrix, inverse_matrix, unit_cell_volume, probe_diameter=1.0, nSample=20, total=0,
    ...
    for i, elem in enumerate(atom_type):
        ...
        for attempt in range(nSample):
            ...
            overlap = CheckSurfaceAreaOverlap(pAtom, pos, cell_matrix, inverse_matrix, atom_type, probe_diameter,
                                              pos[i])
            if not overlap:
                counted += 1
...
def get_surface(cifname):
    ...
    vol, surface_area, a = vsa(atom_type, pos, cell_matrix, inverse_matrix, unit_cell_volume, probe_diameter=3.72,
                               nSample=200)
...
def collect_result(result):
    # Callback function
    surface_data_presort.append(result)


def one_mof(round_idx):
    mof_name = data.iloc[round_idx]['MOFname'] + '.cif'
    ic(mof_name)
    cifname = os.path.join(root_folder, 'data', 'mof_cif_{}'.format(dataset_name), mof_name)

    # Call the get_surface function, the sole reason why we are here in the first place
    vol, surface_area, a = get_surface(cifname)
    result = {"MOFname": mof_name, "vsa": vol, "surface_area": surface_area, "atoms": a}
    return result


def main():
    # Split the whole calculation in to a smaller part, defined by round_number and max_round
    # Each round except the last round will have equal number of samples to work with
    # with the last round is the remainder
    ...
    try:
        ...
        if round_number != max_round:
            ...
            round_end_idx = round_start_idx + len(data.index) // (max_round - 1)
        else:
            round_start_idx = len(data.index) // (max_round - 1) * (round_number - 1)
            round_end_idx = len(data.index)
        pass
    except IndexError:
        round_number = 1
        round_start_idx = 0
    ...
    # Implement Parallelize using Pool.apply()
    pool = mp.Pool(mp.cpu_count())

    # Let's start with asynchronous calculation first
    ...
    # Sort results
    surface_data = pd.DataFrame(surface_data_presort).sort_values(by="MOFname", ignore_index=True)
    surface_data.to_csv(os.path.join(root_folder, "data", "clean_{}_{}_{}.csv".format(dataset_name, round_number,
                                                                                      max_round)))
