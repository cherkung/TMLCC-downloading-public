...
def main():
    ...
    # # pores_table_presort = pool.map_async(one_mof, range(round_start_idx, round_end_idx))
    # for round_idx in range(round_start_idx, round_end_idx):
    ...
    with ProcessPool() as pool:
        future = pool.map(one_mof, range(round_start_idx, round_end_idx), timeout=120)
        ...
        while True:
            try:
                ...
                ic('function took longer than {} seconds'.format(error.args[1]))
            except ProcessExpired as error:
                ic('{}. Exit code {}'.format(error, error.exitcode))
            except Exception as error:
                ic('function raised {}'.format(error))
                ic(error.__traceback__)

    pores_table = pd.DataFrame(pores_table_presort).sort_values(by='MOFname', ignore_index=True)
    pores_table.to_csv(os.path.join(root_repo_folder, 'data', 'pores_{}_{}_{}.csv'.format(dataset_name, round_number,
                                                                                          max_round)))
